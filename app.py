...
st.set_page_config(
    page_title="CV Project • Faster-RCNN Team", 
    page_icon="🚀", 
    layout="wide"
)
...
with st.sidebar:
    ...
    page = st.radio(
        ...
        [
            ...
            "✳️ Сегментация космоснимков"
        ]
    )
    st.markdown("---")
    st.markdown("### 👥 Наша команда:")
    st.markdown("**Faster-RCNN Team**")
    st.caption("Разработчик 1: Алина (Лица)")
    st.caption("Разработчик 2: Напарник (Ветряки, Сегментация)")
...
if page == "🏠 Главная страница":
    ...
    st.subheader("Веб-сервис искусственного интеллекта команды detecron")
    
    # Отображение логотипа команды, если он есть в папке images
    if os.path.exists("images/logo.jpg"):
        st.image("images/logo.jpg", width=400, caption="detecron Team Logo")
    ...
    1. **Детекция и маскирование лиц:** Автоматическое обнаружение лиц на базе обученной модели **YOLOv11 Nano** с последующим конфиденциальным размытием Гаусса (OpenCV). Поддерживает множественную загрузку файлов и инференс по URL-ссылке.
    2. **Детекция ветрогенераторов:** Поиск промышленных объектов на аэрофотоснимках с использованием архитектуры **YOLOv11/12**, с поддержкой подгрузки изображений по прямым интернет-ссылкам.
